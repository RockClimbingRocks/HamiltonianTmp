...
function AnaliticalNormOfSYK4Averaged(L::Int)
    norm2 = L*(L-2)*(L^2 + 6L + 8 - 2*(L-2)/(L-1)) / 4 ;
    return √norm2;
end

function NumericalNormOf__()
...
function c⁺c⁺c⁻c⁻_create_(L::Int64, U::Array{Float64, 4}, isSparse:: Bool)
    N::Int64 = Int(L÷2);
    ...
    for i=1:L, j=i+1:L, k=i:L, l= l̂(i,j,k):L
        U[L+1-i,L+1-j,L+1-k,L+1-l] ≈ 0 ? continue : nothing; 
        ...
        vals_ij = map(elm -> sign(ind[elm[2]]-1, i, j, k, l, tmp_storingVector) * 4*U[L+1-i,L+1-j,L+1-k,L+1-l], matrixElements); 
    ...
    normalization = AnaliticalNormOfSYK4Averaged(L);
    vals ./= normalization
